      ...
      <el-row v-for="item in ruleListDataC" :key="item.field_name">
        <el-col :span="24" class="grid-cell">
          <template v-if="item.field_name == 'annual_review'">
            <el-form-item label="单独年审：" class="label-right-align">
              <span>{{ item.price }}</span>
              <!-- <el-input :value="item.price" type="text" disabled></el-input> -->
            </el-form-item>
          </template>
          <template v-if="item.field_name == 'agent_address'">
            <el-form-item label="注册代理人地址：" prop="agentAddress" class="label-right-align">
              <el-radio-group v-model="formData.agentAddress">
                <el-radio
                  :value="0"
                  :label="0"
                  style="
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const getRuleList = () => {
      ...
      ruleList({ page: 1, count: 20, businessType: 20, region: region, company_type: 'C' }).then((res: any) => {
      ...
      ruleList({ page: 1, count: 20, businessType: 20, region: region, company_type: 'LLC' }).then((res: any) => {
        const list = res.data.toSorted((a, b) => {
          if (a.order > b.order) {
            return 1
          } else {
    ...
    const computedTotalPrice = () => {
      ...
      if (state.formData.companyType == 1) {
        ruleListDataC.value.map((item) => {
          // 相乘
          if (item.field_name == 'annual_review') {
            price1 += item.price
          } else if (item.field_name == 'agent_address') {
            if (state.formData.agentAddress) {
              price1 += item.price
            }
          }
        })
