<template>
  ...
  <el-form :model="formData" ref="vForm" :rules="rules" label-position="right" label-width="180px" size="default" @submit.prevent>
    ...
    <el-row>
            ...
            <div class="table-container">
              <table class="table-layout">
                <tbody>
                  ...
                  <template v-for="(item, index) in formData.ruleListDataC">
                    <tr v-if="item.option">
                      <td class="table-cell">
                        <el-radio
                          :key="index"
      ...
      <template v-if="formData.companyType" v-for="(item, index) in formData.ruleListDataC">
        <el-col :span="24" v-if="!item.option" class="grid-cell">
          ...
          <el-form-item v-if="item.field_name == 'total_asset'" label="公司资产总额：" prop="total_asset" class="required label-right-align">
            <el-input v-model="formData.total_asset" type="text" clearable>
          ...
          <el-form-item v-if="item.field_name == 'amount_fixed_asset'" label="固定资产金额：" prop="amount_fixed_asset" class="required label-right-align">
            <el-input @change="computedTotalPrice" v-model="formData.amount_fixed_asset" type="text" clearable :placeholder="'注：费用为' + item.price">
              <template #append>万美元</template>
            </el-input>
          </el-form-item>
...
<script lang="ts">
import { defineComponent, toRefs, reactive, getCurrentInstance, onMounted, ref, watch } from 'vue'
  ...
  setup(props, context) {
    ...
    const getRuleList = () => {
      ...
      ruleList({ page: 1, count: 20, businessType: 10, region: region, company_type: 'C' }).then((res: any) => {
        ...
        state.formData.ruleListDataC = list
      })
      ruleList({ page: 1, count: 20, businessType: 10, region: region, company_type: 'LLC' }).then((res: any) => {
        ...
        state.formData.ruleListDataLLC = list
      })
    }
    const regionChange = () => {
      // getRuleList()
    ...
    const order = () => {
      state.formData.ruleListDataC = ruleListDataCCCache.value.sort((item) => {
    ...
    const computedTotalPrice = () => {
      let price1 = 0
      state.formData.ruleListDataC.map((item) => {
        // 相乘
      ...
      if (state.formData.companyType == 2) {
        if (state.formData.is_c_rule) {
          if (state.formData.first_year) {
            let item = state.formData.ruleListDataLLC.find((item) => {
      ...
      state.formData.totalPrice = price1 + state.formData.companyMainIncome.price
